...
from xnmt.eval import evaluators
...
class TestBLEU(unittest.TestCase):
  ...
  def test_bleu_1gram(self):
    bleu = evaluators.BLEUEvaluator(ngram=1)
    exp_bleu = 3.0 / 5.0
  ...
  def test_bleu_4gram_fast(self):
    bleu = evaluators.FastBLEUEvaluator(ngram=4, smooth=1)
...
class TestGLEU(unittest.TestCase):
  def setUp(self):
    self.evaluator = evaluators.GLEUEvaluator()
...
class TestSequenceAccuracy(unittest.TestCase):
  def setUp(self):
    self.evaluator = evaluators.SequenceAccuracyEvaluator()
...
class TestSegmentationFMeasureEvaluator(unittest.TestCase):
  def test_fmeasure(self):
    self.assertEqual(evaluators.SegmentationFMeasureEvaluator().evaluate_one_sent("ab c def".split(), "a bc def".split()).value(),
                     0.80)

  def test_fmeasure_error(self):
    with self.assertRaises(Exception) as context:
      evaluators.SegmentationFMeasureEvaluator().evaluate_one_sent("aaa b".split(), "aaa".split())
