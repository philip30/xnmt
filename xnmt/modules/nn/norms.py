...
class BatchNorm(xnmt.Serializable, models.Transform, models.SeqTransducer):
  ...
  def __init__(self,
               ...
               population_running_std: Optional[np.ndarray] = None,
               bn_momentum: float = 0.1,
               bn_epsilon: float = 0.1):
  ...
  def get_normalizer_dimensionality(self) -> Sequence[int]:
    if self.num_dim == 1:
  ...
  def get_stat_dimensions(self) -> List[int]:
    if self.time_first:
      return list(range(self.num_dim-1))
    else:
      return list(range(1, self.num_dim))

  def transform(self, input_expr: dy.Expression, mask: Optional[xnmt.Mask]=None) -> dy.Expression:
    """
    Apply batch norm.
    ...
    if self.train:
      ...
      self.population_running_mean += (-self.bn_momentum) * self.population_running_mean + self.bn_momentum * bn_mean.npvalue()
      bn_std = dy.std_dim(input_expr, self.get_stat_dimensions(), True, num_unmasked)
      self.population_running_std += (-self.bn_momentum) * self.population_running_std + self.bn_momentum * bn_std.npvalue()
    else:
    ...
    bn_xhat = dy.cdiv(bn_numerator, dy.reshape(bn_std, self.get_normalizer_dimensionality()) + self.bn_epsilon)
  ...
  def transduce(self, es: xnmt.ExpressionSequence) -> xnmt.ExpressionSequence:
    output = self.transform(es.as_tensor(), es.mask)
    return xnmt.ExpressionSequence(expr_tensor=output, mask=es.mask)


# Batch norm helpers:
def broadcast_factor(mask: xnmt.Mask, tensor_expr: dy.Expression):
  """
...
def mask_reshape_size(mask: xnmt.Mask, tensor_dim: tuple, time_first: bool = False) -> List[int]:
  if time_first:
...
def set_masked_to_mean(mask: xnmt.Mask, tensor_expr: dy.Expression, time_first: bool = False) -> dy.Expression:
  """
