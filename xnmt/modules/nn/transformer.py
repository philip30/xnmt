...
from xnmt.modules.nn.transforms import Linear
...
class LinearNoBiasSent(object):
  ...
  def __call__(self, input_expr):
    (_, seq_len), batch_size = input_expr.dim()
    output = self.L.transform(input_expr)
...
class MultiHeadAttention(object):
  ...
  """
  def __init__(self, dy_model, n_units, h=1, attn_dropout=0.0):
  ...
  def __call__(self, x, z=None, mask=None):
    ...
    batch_A = dy.cmult(batch_A, mask) + (1 - mask)*-xnmt.globals.INF
...
class EncoderLayer(object):
  def __init__(self, dy_model, n_units, h=1, dropout=0.0, layer_norm=False):
    self.self_attention = MultiHeadAttention(dy_model, n_units, h, attn_dropout=dropout)
  ...
  def __call__(self, e, xx_mask):
    ...
    if self.dropout != 0.0 and xnmt.is_train():
...
class DecoderLayer(object):
  def __init__(self, dy_model, n_units, h=512, dropout=0.0, layer_norm=False):
    self.self_attention = MultiHeadAttention(dy_model, n_units, h, attn_dropout=dropout)
    self.source_attention = MultiHeadAttention(dy_model, n_units, h, attn_dropout=dropout)
    self.feed_forward = FeedForwardLayerSent(dy_model, n_units)
    self.layer_norm = layer_norm
    self.dropout = dropout
...
class TransformerEncoder(xnmt.Serializable):
  ...
  def __init__(self, layers=1, input_dim=512, h=1, dropout=0.0, attn_dropout=False, layer_norm=False):
    ...
    for i in range(1, layers + 1):
      name = 'l{}'.format(i)
      layer = EncoderLayer(dy_model, input_dim, h, attn_dropout, layer_norm, dropout)
...
class TransformerDecoder(xnmt.Serializable):
  ...
  def __init__(self, layers=1, input_dim=512, h=1,
               ...
               trg_reader = xnmt.ref_trg_reader):
    dy_model = xnmt.param_manager(self)
    self.layer_names = []
    for i in range(1, layers + 1):
      name = 'l{}'.format(i)
      layer = DecoderLayer(dy_model, input_dim, h, dropout, layer_norm)
      self.layer_names.append((name, layer))
  ...
  def output(self, h_block):
    return self.output_affine(h_block, reconstruct_shape=False, timedistributed=True)
