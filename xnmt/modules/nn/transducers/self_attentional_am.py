"""
This module implements self-attentional acoustic networks as described in the following paper:
...
class SAAMPositionwiseFeedForward(xnmt.xnmt.Serializable):
  ...
  def __init__(self, input_dim: int, hidden_dim: int, nonlinearity: str = "rectify",
               ...
               layer_norm: Optional[norms.LayerNorm] = None) -> None:
    ...
    self.w_1 = w_12[0]
    self.w_2 = w_12[1]
    self.layer_norm = self.add_serializable_component("layer_norm", layer_norm, lambda: xnmt.modules.nn.transformer.LayerNorm(input_dim))
...
class SAAMMultiHeadedSelfAttention(xnmt.xnmt.Serializable):
  ...
  def __init__(self, head_count: int, model_dim: int, downsample_factor: int = 1, input_dim: int = None,
               ...
               param_init: xnmt.models.templates.ParamInitializer = xnmt.param_initializers.GlorotInitializer(),
               bias_init: xnmt.models.templates.ParamInitializer = xnmt.param_initializers.ZeroInitializer(),
    ...
    self.layer_norm = self.add_serializable_component("layer_norm", layer_norm, lambda: xnmt.modules.nn.transformer.LayerNorm(model_dim))
  ...
  def __call__(self, x: dy.Expression, att_mask: np.ndarray, batch_mask: np.ndarray, p: float):
    ...
    if self.downsample_factor > 1:
      ...
      x = xnmt.ExpressionSequence(expr_tensor=dy.reshape(x.as_tensor(), (
    ...
    if self.kq_pos_encoding_type is None:
      ...
      kq_lin = self.linear_kq.transform(
        SAAMTimeDistributed()(
          xnmt.ExpressionSequence(expr_tensor=dy.concatenate([x.as_tensor(), encoding]))))
    ...
    if xnmt.globals.singleton_global.LOG_ATTENTION:
      xnmt.yaml_logger.info({"key": "selfatt_mat_ax0", "value": np.average(attn.value(), axis=0).dumps(), "desc": self.desc})
      xnmt.yaml_logger.info({"key": "selfatt_mat_ax1", "value": np.average(attn.value(), axis=1).dumps(), "desc": self.desc})
      xnmt.yaml_logger.info({"key": "selfatt_mat_ax0_ent", "value": entropy(attn.value()).dumps(), "desc": self.desc})
      xnmt.yaml_logger.info({"key": "selfatt_mat_ax1_ent", "value": entropy(attn.value().transpose()).dumps(), "desc": self.desc})
...
class TransformerEncoderLayer(xnmt.xnmt.Serializable):
  ...
  def __init__(self,
               ...
               downsample_factor = 1,
               input_dim = None,
               diagonal_mask_width = None,
               ignore_masks = False,
               plot_attention =None,
               nonlinearity = "rectify",
               diag_gauss_mask = False,
               square_mask_std = True,
               cross_pos_encoding_type = None,
               ff_lstm = False,
               kq_pos_encoding_type = None,
               kq_pos_encoding_size = 40,
               max_len = 1500,
               param_init = xnmt.default_param_init,
               bias_init = xnmt.default_bias_init,
               dropout = None,
               self_attn = None,
               feed_forward = None,
               desc = None):
    self.self_attn = self.add_serializable_component("self_attn",
                                                     self_attn,
                                                     lambda: SAAMMultiHeadedSelfAttention(head_count,
                                                                                          hidden_dim,
                                                                                          downsample_factor,
                                                                                          input_dim=input_dim,
    ...
    if ff_lstm:
      self.feed_forward = self.add_serializable_component("feed_forward",
                                                          feed_forward,
                                                          lambda: xnmt.modules.transducers.BiLSTMSeqTransducer(
                                                            layers=1,
      ...
      self.feed_forward = self.add_serializable_component("feed_forward",
                                                          feed_forward,
                                                          lambda: SAAMPositionwiseFeedForward(
                                                            input_dim=hidden_dim,
  ...
  def transduce(self, x: xnmt.ExpressionSequence) -> xnmt.ExpressionSequence:
    seq_len = len(x)
    batch_size = x[0].dim()[1]
    ...
    if self.ff_lstm:
      ...
      out = self.feed_forward.transduce(xnmt.xnmt.ExpressionSequence(expr_tensor=mid_re, mask=out_mask))
      out = dy.reshape(out.as_tensor(), (hidden_dim,), batch_size=seq_len * batch_size)
    else:
      out = self.feed_forward.transduce(mid, p=self.dropout)


    return xnmt.ExpressionSequence(expr_tensor=dy.reshape(out, (out.dim()[0][0], seq_len), batch_size=batch_size),
                                   mask=out_mask)
...
class SAAMSeqTransducer(xnmt.models.SeqTransducer, xnmt.Serializable):
  ...
  def __init__(self,
               ...
               diagonal_mask_width: int = None,
               ignore_masks: bool = False,
               plot_attention: Optional[str] = None,
               nonlinearity: str = "rectify",
               pos_encoding_type: Optional[str] = None,
               pos_encoding_combine: str = "concat",
               pos_encoding_size: int = 40,
               max_len: int = 1500,
               diag_gauss_mask: Union[bool, float] = False,
               square_mask_std: bool = True,
               cross_pos_encoding_type: Optional[str] = None,
               ff_lstm: bool = False,
               kq_pos_encoding_type: Optional[str] = None,
               ...
               modules = None):
    self.input_dim = input_dim + (pos_encoding_size if (pos_encoding_type and pos_encoding_combine == "concat") else 0)
      ...
      self.positional_embedder = \
        self.add_serializable_component("positional_embedder",
                                        ...
                                        lambda: embedders.PositionEmbedder(max_pos=self.max_len,
                                                                           emb_dim=self.input_dim if self.pos_encoding_combine == "add" else self.pos_encoding_size))
  ...
  def make_modules(self,
    ...
    for layer_i in range(layers):
      ...
      modules.append(TransformerEncoderLayer(hidden_dim,
                                             downsample_factor=downsample_factor,
                                             input_dim=self.input_dim if layer_i == 0 else hidden_dim,
  ...
  def transduce(self, sent: xnmt.ExpressionSequence) -> xnmt.models.states.EncoderState:
    ...
    if self.pos_encoding_type:
      if self.pos_encoding_combine == "add":
        sent = xnmt.ExpressionSequence(expr_tensor=sent.as_tensor() + encoding, mask=sent.mask)
      else:  # concat
        sent = xnmt.ExpressionSequence(expr_tensor=dy.concatenate([sent.as_tensor(), encoding]),
                                                      mask=sent.mask)
    ...
    for module in self.modules:
      ...
      sent = enc_sent
    final_states = [xnmt.models.states.FinalTransducerState(sent[-1])]
    return xnmt.models.states.EncoderState(sent, final_states)

  # TODO Make this object stateless if possible
