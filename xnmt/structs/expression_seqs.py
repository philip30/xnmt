...
class ExpressionSequence(object):
  ...
  def __init__(self,
               ...
               mask: Optional[batch.Mask] = None) -> None:
    """Constructor.
...
class LazyNumpyExpressionSequence(ExpressionSequence):
  ...
  """
  def __init__(self, lazy_data: np.ndarray, mask: Optional[batch.Mask] = None) -> None:
    """
    Args:
  ...
  def __len__(self):
    if self.expr_list or self.expr_tensor:
      ...
      if batch.is_batched(self.lazy_data):
  ...
  def as_tensor(self) -> dy.Expression:
    if not (self.expr_list or self.expr_tensor):
      if not batch.is_batched(self.lazy_data):
        raise NotImplementedError()
      array = np.concatenate([d.get_array().reshape(d.get_array().shape + (1,)) for d in self.lazy_data], axis=2)
      self.expr_tensor = dy.inputTensor(array, batched=batch.is_batched(self.lazy_data))
...
class ReversedExpressionSequence(ExpressionSequence):
  ...
  """
  def __init__(self,
               base_expr_seq: ExpressionSequence = None,
               expr_tensor: dy.Expression = None,
               expr_list: List[dy.Expression] = None,
